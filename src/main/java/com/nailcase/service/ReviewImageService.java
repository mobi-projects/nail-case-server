...
public class ReviewImageService extends ImageService<ReviewImage> {
	...
	public ReviewImageService(AmazonS3 amazonS3,
	...
	public CompletableFuture<ImageDto> saveImageAsync(MultipartFile file, ReviewImage image, Authentication auth) {
		return CompletableFuture.supplyAsync(() -> {
			try {
				return super.saveImageAsync(file, image, reviewImageRepository, auth).get();
			} catch (Exception e) {
	...
	public CompletableFuture<List<ImageDto>> saveImagesAsync(List<MultipartFile> files, List<ReviewImage> images,
		...
		return CompletableFuture.supplyAsync(() -> {
			try {
				return super.saveImagesAsync(files, images, reviewImageRepository, auth).get();
			} catch (Exception e) {
